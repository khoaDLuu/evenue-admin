        ...
        <table class="table-auto w-full">
          ...
          <thead class="text-xs font-semibold uppercase text-gray-400 bg-gray-50">
            <tr>
              ...
              <th class="p-2 whitespace-nowrap">
                <div class="font-semibold text-center">Date Joined</div>
              </th>
            </tr>
          </thead>
          <!-- Table body -->
              ...
              <td class="p-2 whitespace-nowrap">
                <div class="flex items-center">
                  <!-- <div class="w-10 h-10 shrink-0 mr-2 sm:mr-3">
                    <img class="rounded-full" :src="user.image" width="40" height="40" :alt="user.name" />
                  </div> -->
                  <div class="font-medium text-gray-800">{{user.name}}</div>
                </div>
              </td>
              <td class="p-2 whitespace-nowrap">
                  <a class="text-blue-400" :href="`mailto:${ user.email }`" target="_blank">
                    {{ user.email }}
                  </a>
              </td>
              <td class="p-2 whitespace-nowrap">
                <div class="text-left font-medium text-green-500">{{user.phone}}</div>
              </td>
              <td class="p-2 whitespace-nowrap">
                <div class="text-lg text-center">{{user.createdAt}}</div>
              </td>
            </tr>
          </tbody>
        </table>
        ...
        <div class="hidden sm:flex-1 sm:flex sm:items-center sm:justify-between">
          <div>
            <p class="text-sm text-gray-700">
              Showing
              <span class="font-medium">{{ users.length ? 1 : 0 }}</span>
              to
...
import Dummy from '../../images/user-36-05.svg'
  ...
  computed: {
    usersShown() {
      return this.users.map(u => ({
        id: u.Attributes.filter(a => a.Name == "sub").map(a => a.Value)?.[0],
        name: u.Attributes.filter(a => a.Name == "name").map(a => a.Value)?.[0],
        email: u.Attributes.filter(a => a.Name == "email").map(a => a.Value)?.[0],
        phone: u.Attributes.filter(a => a.Name == "phone_number").map(a => a.Value)?.[0],
        createdAt: new Date(u.UserCreateDate).toLocaleDateString(),
      }))
    }
  }
}
